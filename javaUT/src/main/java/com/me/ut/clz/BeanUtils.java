...
public class BeanUtils
{
    ...
    public static <T> T deepCopyBean(T source)
    {
        ...
        try
        {
            ...
            PropertyDescriptor[] origDescriptors = bean.getPropertyUtils()
            ...
            for (int i = 0; i < origDescriptors.length; i++)
            {
                        ...
                        && bean.getPropertyUtils().isWriteable(dest, name))
                {
                    try
                    {
                        Object value = deepCopyBean(bean.getPropertyUtils()
                                .getSimpleProperty(source, name));
                        bean.getPropertyUtils().setSimpleProperty(dest, name,
                                value);
            ...
            ArrayIndexOutOfBoundsException, IllegalArgumentException
    {
        ...
        Object dest = Array.newInstance(source.getClass().getComponentType(),
                length);
        if (length == 0)
        {
            return dest;
    ...
    @SuppressWarnings({"unchecked"})
    private static Map deepCopyMap(Map source) throws InstantiationException,
